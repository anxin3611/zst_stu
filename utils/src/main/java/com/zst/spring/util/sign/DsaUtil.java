...
public class DsaUtil {
    ...
    public static void plantWide(SignEnum signEnum) throws Exception {
        ...
        // 2. 签名
        byte[] sign = sign(STR_MSG, (DSAPrivateKey) keyPair.getPrivate(), signEnum);
     ...
     *
     * @param data          签名数据
     * @param dsaPrivateKey dsaPrivateKey
     * @param signEnum      签名的参数
     * @return byte[] 签名后二进制的数据
     */
    public static byte[] sign(String data, DSAPrivateKey dsaPrivateKey, SignEnum signEnum) throws Exception {
        Signature signature = Signature.getInstance(signEnum.signType);
        signature.initSign(SecurityUtils.getPrivateKey(dsaPrivateKey.getEncoded(), signEnum));
     ...
     * @return boolean 验证结果
     */
    public static boolean verify(byte[] sign, byte[] rsaPublicKey, SignEnum signEnum) throws NoSuchAlgorithmException, InvalidKeySpecException, InvalidKeyException, SignatureException {
        Signature signature = Signature.getInstance(signEnum.signType);
